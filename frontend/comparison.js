(function () {
  ...
  function render(current) {
    ...
    const rows = fieldDefinitions.map(([key,label]) => {
      ...
      return `<div class="compare-row ${needsReview ? 'unresolved' : flagged ? 'flagged' : ''}" role="row"><div role="rowheader" class="field-name">${label}</div><div role="cell" class="field-value"><strong>${escapeHTML(pair[0].raw)}</strong><a href="evidence.html?case=${encodeURIComponent(current.id)}&field=${key}">View Shipping Instruction source</a></div><div role="cell" class="field-value"><strong>${escapeHTML(pair[1].raw)}</strong><a href="evidence.html?case=${encodeURIComponent(current.id)}&field=${key}">View Bill of Lading source</a></div><span role="cell" class="pill ${needsReview || flagged ? 'orange' : 'teal'}">${needsReview ? 'Needs review' : flagged ? 'Mismatch' : 'Match'}</span></div>`;
    }).join('');
    surface.innerHTML = `<div class="case-banner"><div><span class="kicker">${escapeHTML(current.id)} · Comparison complete</span><h2>${escapeHTML(current.subject)}</h2><p>${escapeHTML(current.siFile)} compared with ${escapeHTML(current.blFile)}</p></div><span class="case-result ${differences.length ? 'mismatch' : 'match'}">${differences.length ? `${differences.length} mismatch detected` : 'No mismatch detected'}</span></div><div class="comparison-table" role="table" aria-label="SI and BL field comparison"><div class="compare-row header" role="row"><div role="columnheader">Field</div><div role="columnheader">Shipping instruction</div><div role="columnheader">Draft bill of lading</div><div role="columnheader">Result</div></div>${rows}</div><div class="surface-foot"><span>Completed in ${escapeHTML(current.processingTime)} · ${current.evidenceCoverage} / 7 fields have source evidence</span><a class="secondary-button" href="activity.html?case=${encodeURIComponent(current.id)}">Processing activity</a></div>`;
  }
